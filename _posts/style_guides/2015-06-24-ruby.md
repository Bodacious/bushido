...
To make it easier, we've added {% label different %} tags to the ones where our style differs.
...
### Use consistent class naming conventions  {% label different %} 
...
### Avoid single-line methods  {% label different %}
...
This rule also applies to empty-body methods. {% label different %}
...
### No space inside range literals {% a no-space-inside-range-literals %}
...
### Indenting `when` in `case` statements {% a  indent-when-to-case %}
...
### Indenting conditional assigment {% a indent-conditional-assignment %}
...
### Empty lines between methods {% a empty-lines-between-methods %} {% label different %}
...
### Avoid a comma after the last paramter {% a  no-trailing-params-comma %}
...
### Chaining methods across multiple lines {% label different %}
...
### Avoid new lines at the end of files {% label different %}
...
### Use the ternary for short conditions {% label different %}
...
### Keep conditional branches as clear as possible {% label different %}
...
### Use  `!!` when required {% label different %}
